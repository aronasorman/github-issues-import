...
def format_date(datestring):
	...
	date_format = config.get('format', 'date') if config.has_option('format', 'date') else '%A %b %d, %Y at %H:%M GMT';
...
def format_issue(template_data):
	...
	template = config.get('format', 'issue_template')  if config.has_option('format', 'issue_template') else default_template
...
def format_pull_request(template_data):
	...
	template = config.get('format', 'pull_request_template')  if config.has_option('format', 'pull_request_template') else default_template
	return format_from_template(template, template_data)

def format_comment(template_data):
	default_template = os.path.join(__location__, 'templates', 'comment.md')
	template = config.get('format', 'comment_template')  if config.has_option('format', 'comment_template') else default_template
	return format_from_template(template, template_data)
...
def import_issues(issues):
	...
	if raw_input("Are you sure you wish to continue?") not in ["", 'y', 'Y']:
		sys.exit()
